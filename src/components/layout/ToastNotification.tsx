...
export function ToastNotification({ toast, onClose }: ToastNotificationProps) {
	...
	return (
		<div
			className={`
      fixed bottom-6 right-6 max-w-sm w-full border-2 p-4 rounded shadow-2xl flex gap-3 animate-telegram-toast z-50 justify-between items-start
		...
		>
			...
			{onClose && (
				<button
					type="button"
					onClick={onClose}
					className="text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-100 transition-colors p-0.5 rounded"
